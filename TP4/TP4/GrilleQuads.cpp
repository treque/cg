...
CGrilleQuads::CGrilleQuads(char* nomFichierTexture, float largeur, float hauteur, int nbColonnes, int nbRangees,
                           float echelle, bool mipmap, bool bumpy)
    : CModele3DAbstrait(echelle, nomFichierTexture, TYPE_TEXTURE2D, mipmap)
{
    // initialisations...
    largeur_    = largeur;
    hauteur_    = hauteur;
...
CGrilleQuads::CGrilleQuads(std::vector<char*>* nomsFichiersTextures, float largeur, float hauteur, int nbColonnes,
                           int nbRangees, float echelle, bool mipmap, bool bumpy)
    : CModele3DAbstrait(echelle, nomsFichiersTextures, TYPE_TEXTURE2D, mipmap)
{
    // initialisations...
    largeur_    = largeur;
    hauteur_    = hauteur;
...
void CGrilleQuads::creerLaGrille(float largeur, float hauteur, int nbRangees, int nbColonnes, bool normaleVersExt)
{
    ...
    float incrementX = largeur / nbColonnes;
    float incrementY = hauteur / nbRangees;
    ...
    for (unsigned int i = 0; i < m_sommets.size() / 8; i++)
    {
    ...
    unsigned int s_byte_size = static_cast<unsigned int>(m_sommets.size() * sizeof(float));
    unsigned int i_byte_size = static_cast<unsigned int>(indices.size() * sizeof(int));
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float),
                          reinterpret_cast<char*>(2 * sizeof(float))); // normals
    // glEnableVertexAttribArray(1);
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float),
                          reinterpret_cast<char*>(5 * sizeof(float))); // positions

...
std::vector<std::string> StringUtils::Tokenize(const std::string& str, const std::string& delimiters)
{
...
///
////////////////////////////////////////////////////////////////////////
std::vector<std::string> StringUtils::Tokenize2(const std::string& str, const std::string& delimiters)
{
    std::vector<std::string> tokens;
    ...
    {
        const auto count = str.substr(0, lastPos - pos).length();
    ...
    while (std::string::npos != pos || std::string::npos != lastPos)
    {
        ...
        {
            const auto count = str.substr(pos, lastPos - pos).length();
...
std::string StringUtils::TrimRight(const std::string& source, const std::string& t /*= " " */)
{
    std::string str = source;
    return str.erase(str.find_last_not_of(t) + 1);
}

std::string StringUtils::TrimLeft(const std::string& source, const std::string& t /*= " " */)
{
    std::string str = source;
    return str.erase(0, source.find_first_not_of(t));
}

std::string StringUtils::Trim(const std::string& source, const std::string& t /*= " " */)
{
    std::string str = source;
    return TrimLeft(TrimRight(str, t), t);
}

void StringUtils::FindAndReplace(std::string& source, const std::string& find, const std::string& replace)
{
    size_t j;
...
{
    long sum = 0;
    for (std::string::const_iterator c = rhs.begin(), end = rhs.end(); c != end; ++c)
    {
        if (*c == '0')
        {
            sum <<= 1;

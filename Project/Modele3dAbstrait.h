...
public:
    ...
    CModele3DAbstrait(const float echelle, const char* nomFichierTexture, const TEXTypeEnum texType, const bool mipmap = false);
    /// constructeur créant un modèle 3D générique comportant plusieurs textures
    CModele3DAbstrait(const float echelle, const std::vector<char*>* nomsFichiersTextures, const TEXTypeEnum texType, const bool mipmap = false);

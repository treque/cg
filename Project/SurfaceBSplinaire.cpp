...
CSurfaceBSplinaire::CSurfaceBSplinaire(uint numPts_U, uint numPts_V, const Point* Points, uint num_knots_U,
                                       uint num_knots_V, const float* knots_U, const float* knots_V, uint NDD,
                                       uint degree_U, uint degree_V, uint order_U, uint order_V, float scale_U,
                                       float scale_V)
{
    // initialisations
...
void CSurfaceBSplinaire::setupVAO()
{
    ...
    for (int i = 0; i != NDD_ - 1; ++i)
    {
        for (int j = 0; j != NDD_ - 1; ++j)
        {
            for (int k = i; k <= i + 1; k++)
            {
                for (int l = j + (k == i ? 0 : 1); k == i ? l <= j + 1 : l >= j; k == i ? l++ : l--)
                {
                    ...
                    float ug = (MaxU() - MinU()) * (float)(std::max(k - 1, 0)) / (NDD_ - 1) + MinU();
                    float ud =
                        (MaxU() - MinU()) * (float)(std::min((unsigned int)k + 1, NDD_ - 1)) / (NDD_ - 1) + MinU();
                    ...
                    float vh = (MaxV() - MinV()) * (float)(std::max(l - 1, 0)) / (NDD_ - 1) + MinV();
                    float vb =
                        (MaxV() - MinV()) * (float)(std::min((unsigned int)l + 1, NDD_ - 1)) / (NDD_ - 1) + MinV();
                    ...
                    vectMoyenU = gVect + dVect;
                    vectMoyenU /= ((k == 0) | (l == 0) | (k == NDD_ - 1) | (l == NDD_ - 1)) ? 1.0f : 2.0f;

                    vectMoyenV = bVect + hVect;
                    vectMoyenV /= ((k == 0) | (l == 0) | (k == NDD_ - 1) | (l == NDD_ - 1)) ? 1.0f : 2.0f;
    ...
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (char*)(0)); // texcoords
    // glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (char*)(2 * sizeof(float))); // normals
    // glEnableVertexAttribArray(1);
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (char*)(5 * sizeof(float))); // positions

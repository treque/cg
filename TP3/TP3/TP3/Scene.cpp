...
void CScene::LancerRayons(void)
{
    ...
    CVecteur3 right = CVecteur3::ProdVect(forward, m_Camera.Up);
    REAL d = CVecteur3::Distance(m_Camera.Position, m_Camera.PointVise);
    CVecteur3 pxMin = m_Camera.Position + d * forward - d * tan(m_Camera.Focale * PI/180 ) * right; 
    CVecteur3 pxMax = m_Camera.Position + d * forward + d * tan(m_Camera.Focale * PI / 180) * right;
    CVecteur3 pyMin = m_Camera.Position + d * forward - d * tan(m_Camera.Angle * PI / 180) * m_Camera.Up;
    CVecteur3 pyMax = m_Camera.Position + d * forward + d * tan(m_Camera.Angle * PI/180 ) * m_Camera.Up;
    */
    ...
    for (int j = 0; j < m_ResHauteur; ++j)
    {
        ...
        for (int i = 0; i < m_ResLargeur; ++i)
        {
            ...
            m_InfoPixel.push_back(GLfloat(col.r));
            m_InfoPixel.push_back(GLfloat(col.g));
            m_InfoPixel.push_back(GLfloat(col.b));
        }
    }
...
const CCouleur CScene::ObtenirCouleurSurIntersection(const CRayon& Rayon, const CIntersection& Intersection) const
{
    ...
    for (const auto& uneLumiere : m_Lumieres)
    {
        ...
        if (CVecteur3::ProdScal(LumiereRayon.ObtenirDirection(), Intersection.ObtenirNormale()) > 0)
        {
            ...
            if (CVecteur3::ProdScal(R, Rayon.ObtenirDirection()) > REAL(0.0))
            {

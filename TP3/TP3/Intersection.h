...
public:
    ...
    inline void AjusterNormale(const Math3D::CVecteur3& Normal) { m_Normale = Normal; }

    /// Méthodes d'obtention
    inline ISurface*         ObtenirSurface(void) const { return m_Surface; }
    inline REAL              ObtenirDistance(void) const { return m_Distance; }
    inline Math3D::CVecteur3 ObtenirNormale(void) const { return m_Normale; }
};

...
std::vector<float> make2DNoiseArray(int width, int height, int freq, float amp)
{
    int    i, j;
    double ni[2];
    double inci, incj;
    //    float* noise2DArray;
    std::vector<float> noise2DArray(width * height);
...
std::vector<vect3D> compute2DNormalTexture(const std::vector<float>& texNoise2D, int width, int height)
{
    int    i, j;
    ...
    std::vector<vect3D> texNormal2D(width * height);

...
class CTextureCubemap : public CTextureAbstraite
{
public:
    /// constructeur d'une texture cubemap openGL (de type GL_TEXTURE_CUBE_MAP) à partir du nom de fichier de la texture
    CTextureCubemap(const std::string& nomFichier);
    /// destructeur par défaut
    ~CTextureCubemap(void) override;

private:
    /// charge la texture en mémoire avec la cible GL_TEXTURE_CUBE_MAP
    void chargerTexture() override;
    /// membre privé temporaire servant la conserver les texels lus du fichier
    Image textureImage;

    inline auto lirePixelRGB(const int x, const int y)
    {
        return &textureImage.data[0] + ((textureImage.tailleX * (textureImage.tailleY - 1 - y)) + x) * 3;
    }
};

...
CGrilleQuads::CGrilleQuads(const std::string& nomFichierTexture, float largeur, float hauteur, int nbColonnes,
...
CGrilleQuads::CGrilleQuads(const std::vector<std::string>& nomsFichiersTextures, float largeur, float hauteur,
...
void CGrilleQuads::creerLaGrille(float largeur, float hauteur, int nbRangees, int nbColonnes, bool normaleVersExt)
{
    ...
    std::vector<unsigned int> indices(m_sommets.size() / 8 / 4 * 6);
    for (unsigned int i = 0; i < m_sommets.size() / 8 / 4; i++)
    {
        indices[6 * i + 0] = 4 * i + 0;
        indices[6 * i + 1] = 4 * i + 1;
        indices[6 * i + 2] = 4 * i + 2;
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(2 * sizeof(float))); // normals
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(5 * sizeof(float))); // positions
